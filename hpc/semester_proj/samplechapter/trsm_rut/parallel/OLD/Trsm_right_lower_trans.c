...
void Trsm_right_lower_trans( PLA_Obj A, PLA_Obj B, int nb_alg )
{
  ...
  PLA_Pmvector_create_conf_to( B, PLA_PROJ_ONTO_COL, PLA_ALL_COLS, nb_alg, &B1_dpmv );

  PLA_Pmvector_create_conf_to( B, PLA_PROJ_ONTO_ROW, PLA_ALL_ROWS, nb_alg, &A21_dpmv );
  ...
  while ( b = min( PLA_Obj_length( ABR ), nb_alg ) ){
    ...
    PLA_Obj_vert_split_2( A21_dpmv, b, PLA_DUMMY, &A21_dpmv );

    if ( b < nb_alg ){
      PLA_Obj_vert_split_2( B1_dpmv, b, &B1_dpmv, PLA_DUMMY );
      PLA_Obj_horz_split_2( A21_dpmv, b, &A21_dpmv, PLA_DUMMY );
    }
