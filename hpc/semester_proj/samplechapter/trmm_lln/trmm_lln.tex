...
\index{index}{matrix-matrix multiplication!triangular}%
...
\ChapterAuthor{
\index{author}{Bientinesi, Paolo}
Paolo Bientinesi
\\[0.1in]
\index{author}{van de Geijn, Robert A.}
Robert A. van de Geijn
}

% Add the names of the authors to the table of contents

\addtocontents{toc}{ {by {\bf Paolo Bientinesi and Robert A. van de Geijn} }
}
...
% Notice that in all the current semester projects,
% All matrices must be partitioned into quadrants to expose
% symmetry and/or triangular matrices.

\section{Algorithms that start by partitioning $ L $}
\label{sec:trmm_lln:L}

Let us start by partition matrix $ L $.  Since it is triangular, we
partition it like
\[
...
% file trmm_lln/table.new.tex

\input{trmm_lln/table.new}
...
\footnotesize
\begin{quote}
\listinginput{1}{materials/trmm_lln/lazy-rowlazy/Trmm_left_lower_notrans_unb.c}
\end{quote}
...
\begin{quote}
\listinginput{1}{materials/trmm_lln/lazy-rowlazy/Trmm_left_lower_notrans_blk.c}
\end{quote}
...
\begin{quote}
\index{const}{\tt FLA\un RECURSIVE}%
\listinginput{1}{materials/trmm_lln/rightmoving/Trmm_lln_right_wrt_B_blk.c}
\end{quote}
